...
namespace SsbHedger.UnitTests.Converters
{
    public class IbDateStringConverterShould
    {
        ...
        public void ConvertCorrectly(string dateString, string expectedIbDateString)
        {
            DateTime date = DateTime.Parse(dateString, new CultureInfo("DE-de"));

            var sut = new IbDateStringConverter();
            var result = sut.Convert(date, typeof(string), new object(), CultureInfo.InvariantCulture);
        ...
        public void ConvertBackCorrectly(string ibDateString, string expectedDateString)
        {
            DateTime expectedDate = DateTime.Parse(expectedDateString, new CultureInfo("DE-de"));

            var sut = new IbDateStringConverter();
            var result = sut.ConvertBack(ibDateString, typeof(DateTime), new object(), CultureInfo.InvariantCulture);
